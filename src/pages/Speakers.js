    ...
    <Page no={4} page="speakers" title="Speakers & Panellists">
      ...
      <motion.div className="w-full relative overflow-scroll gap-6 px-6 mt-4 flex flex-col md:flex-row md:flex-wrap h-full md:h-[90%]">
        {speakers?.map((speaker, index) => (
          <div className="flex cursor-default flex-col md:flex-row items-start relative md:w-[48%]  font-mont rounded-lg overflow-hidden text-slate-600 border-indigo-200 hover:bg-indigo-200 transition-all bg-indigo-100 border shadow-md gap-4">
            {user?.role === "admin" && (
              <div className="absolute flex flex-row bg-slate-100 rounded-full px-2 py-1 top-2 right-2 ml-4">
                <PencilIcon
                  onClick={() => setEdit({ edit: true, data: speaker })}
